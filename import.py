...
def run(script_args: List) -> int:
    ...
    query = session.query(func.count(models.Events.event_type)).filter(
        models.Events.event_type == 'state_changed').filter(models.Events.time_fired >= '2018-08-02')
    ...
    while True:
        ...
        if step_start > total_events:
            ...
            break
        query = session.query(models.Events).filter(models.Events.time_fired >= '2018-08-02').filter(
